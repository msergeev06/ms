...
class Application extends Multiton
{
    ...
    private $appParams = [];
    private $arBuffer = [];
    private $arIncludedCSS = [];
    private $arIncludedJS = [];
    /**
    ...
    public function endBufferPage()
    {
        if (defined('SHOW_SQL_WORK_TIME') && SHOW_SQL_WORK_TIME === true)
        {
            ...
            echo '<p>' . $DB->getCountQuery() . ' ' . Lib\Tools::sayRusRight(
                    $DB->getCountQuery(), 'запрос', 'запроса', 'запросов'
                ) . ' за ' . $DB->getAllQueryTime() . ' сек.</p><hr>';
    ...
    public function includeComponent(
        $component, $componentTemplate = '', $componentParams = [], Component $parentComponent = null
    ) {
        ...
        {
            echo '<span style="color:red;">Ошибка подключения компонента "' . $component
                 . '" (компонент не существует)</span><br>';
        ...
        if (file_exists($componentsRoot . '/.description.php'))
        {
            ...
            {
                echo '<span style="color:red;">Ошибка подключения компонента "' . $component
                     . '" (отсутствует имя класса)</span><br>';
        ...
        {
            echo '<span style="color:red;">Ошибка подключения компонента "' . $component
                 . '" (отсутсвует файл .description.php)</span><br>';
        ...
        {
            echo '<span style="color:red;">Ошибка подключения компонента "' . $component
                 . '" (отсутствует файл class.php)</span><br>';
    ...
    public function setRefresh($url = '', $time = 0)
    {
        ...
        $this->arBuffer['refresh'] = '<META HTTP-EQUIV=REFRESH CONTENT="'
                                     . (int)$time
                                     . '; URL='
                                     . '//' . Application::getInstance()->getContext()->getServer()->getHttpHost()
                                     . $url
                                     . '">';
    }
    ...
    protected function initializeContext(array $params)
    {
        ...
        $context->initialize($request, $server, ['env' => $params["env"]]);
