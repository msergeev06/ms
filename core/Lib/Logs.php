...
class Logs
{
     ...
     *                                                  ERROR_CODE - используется
     *                                                  совместно с коллекцией ошибок, задавая
     *                                                  код произошедшей ошибки/сообщения
     *                                                  EXCEPTION - должно содержать объект исключения. Добавляет
     *                                                  специфичные для исключения данные
     * @param ErrorCollection|NULL &$errorCollection    Коллекция ошибок, передается
     *                                                  по ссылке и в нее добавляется новыя
     ...
     * @see        FileLogger
     */
    public static function setCritical(
        $strMessage,
        $arReplace = [],
        ErrorCollection &$errorCollection = null,
        $iErrorNumber = null,
     ...
     * @see        FileLogger
     */
    public static function write2Log(
        $strMessage,
        $nameLog = 'sys',
        $noDate = false,
        string $module = 'core'
     ...
     * @see        FileLogger
     */
    private static function setSpecial(
        $type,
        $strMessage,
        $arReplace = [],
        ErrorCollection &$errorCollection = null,
        ...
        {
            case 'DEBUG'://DEBUG
                Events::runEvents('core', 'OnAddDebugMessageToLog', [$strMessage]);
                $loggerType = 'debug';
                break;
            case 'INFO'://NOTICE
                Events::runEvents('core', 'OnAddNoticeMessageToLog', [$strMessage]);
                $loggerType = 'notice';
                break;
            case 'NOTICE'://NOTICE
                Events::runEvents('core', 'OnAddNoticeMessageToLog', [$strMessage]);
                ...
                break;
            case 'WARNING'://ERROR
                Events::runEvents('core', 'OnAddErrorMessageToLog', [$strMessage]);
                $loggerType = 'error';
                break;
            case 'CRITICAL'://ERROR
                Events::runEvents('core', 'OnAddErrorMessageToLog', [$strMessage]);
