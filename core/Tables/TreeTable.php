...
class TreeTable extends Lib\DataManager
{
    ...
    private static $defaultArSelect
        = [
            'LEFT_MARGIN',
            'RIGHT_MARGIN',
            'DEPTH_LEVEL'
        ...
        */
        ...
        if (
            (
                ...
                && (string)strlen($arNode[$sParentFieldName]) <= 0
            )
            || !isset($arNode[$sParentFieldName])
        ) {
            $sql = "SELECT\n\t"
        ...
        else
        {
            ...
            $query = new Query\QueryBase($sql);
            $res = $query->exec();
            /*			Но мы обновили только те узлы в которых изменяются оба
     ...
     * @return bool|array
     */
    final public static function getChildren(
        $primary,
        $arSelect = [],
        $bActive = false,
        $iDepthLevel = 0
     ...
     * @return bool|array
     */
    final public static function getParents(
        $primary,
        $arSelect = [],
        $bActive = false,
    ...
    protected static function getMap()
    {
        return [
            ...
            (new Fields\IntegerField('PARENT_ID'))
                ...
                ->setLink(static::getTableName() . '.ID')
