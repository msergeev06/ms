...
/**
 * Класс Ms\Core\Entity\Db\Tables\TreeORMController
 * ORM контроллер для деревьев
 */
class TreeORMController extends ORMController
{
    protected static $defaultArSelect =
        [
            'LEFT_MARGIN',
            'RIGHT_MARGIN',
            'DEPTH_LEVEL'
    ...
    /** @var null|TreeTableAbstract */
    protected $table = null;

    /**
     * Защищенный конструктор класса TreeORMController
     *
     * @param TreeTableAbstract $table
     */
    protected function __construct (TreeTableAbstract $table)
    {
        $this->table = $table;
    }
     ...
     * @unittest
     */
    public function changeParent ($primary, $newParentPrimary = null)
    {
        $conn = Application::getInstance()->getConnection();
        $this->clearErrorCollection();
        //OK
        $oPrimaryField = $this->getPrimaryField();
        $sPrimaryFieldName = $oPrimaryField->getColumnName();
        $sParentFieldName = $this->getTable()->getParentFieldName();
        $arNode = $this->getByPrimary($primary, $this->getArSelect());
        $arParent[$sPrimaryFieldName] = $this->getParentPrimary($arNode[$sPrimaryFieldName]);
        ...
        $res = $this->update($primary, [$sParentFieldName => $newParentPrimary]);
        if (!$res->getResult())
            //		if (false)
        {
            $this->getErrorCollection()->add(
                'CHANGE_PARENT_ID',
                'Возникла ошибка на шаге: Для начала сразу записываем ключ нового родителя в узел'
            )
            ;
        ...
        if (is_null($newParentPrimary))
        {
            ...
            {
                $this->getErrorCollection()->add(
                    'LEVEL_NEW_PARENT_NODE',
                    'Возникла ошибка на шаге: 2,3. Уровень нового '
                    . 'родительского узла (если узел перемещается в "корень" то '
                    . 'сразу можно подставить значение 0)'
                )
                ;
        ...
        {
            $arNewParent = static::getByPrimary(
                $newParentPrimary,
                $this->getArSelect()
            );
            $arChild = $this->getChildren($arNewParent[$sPrimaryFieldName]);
            //msDebug($arChild);
            if (count($arChild) > 1)
            {
            ...
            else
            {
                ...
                $sql = "SELECT\n\t"
                       ...
                       . $helper->wrapTableQuotes() . "\n"
                       . "WHERE\n\t"
                       . $helper->wrapFieldQuotes($sPrimaryFieldName) . " = "
                       . $oPrimaryField->getSqlValue(
                        $arNewParent[$sPrimaryFieldName]
                    );
                $query = new QueryBase($sql);
                $res = $query->exec();
                if ($ar_res = $res->fetch())
                ...
                else
                {
                    $this->getErrorCollection()->add(
                        'LEVEL_NEW_PARENT_NODE',
                        'Возникла ошибка на шаге: 2,3. Уровень нового '
                        . 'родительского узла (если не в корень)'
                    )
                    ;
        ...
        if ($oPrimaryField instanceof StringField)
        {
        ...
        if ($arParams['right_key_near'] < $arParams['right_key'])
        {
            ...
            $sql = "UPDATE\n\t"
                   ...
                   . "WHERE\n\t"
                   . $helper->wrapFieldQuotes('RIGHT_MARGIN') . " < "
                   . $arParams['left_key'] . " AND \n\t"
                   . $helper->wrapFieldQuotes('RIGHT_MARGIN') . " > "
                   . $arParams['right_key_near'] . " AND\n\t"
                   . $helper->wrapFieldQuotes($sPrimaryFieldName) . " NOT IN ("
                   . implode(',', $arParams['id_edit'])
                   . ")";
            $query = new QueryBase($sql);
            try
            {
                ...
                if (!$res->isSuccess())
                {
                    $this->getErrorCollection()->add(
                        'GO_UP_6_2',
                        'Возникла ошибка на шаге: Перемещаемся вверх 6.2.'
                    )
                    ;
            ...
            try
            {
                $res = $query->exec();//			msEchoVar($sql);
                if (!$res->isSuccess())
                {
                    $this->getErrorCollection()->add(
                        'GO_UP_6_3',
                        'Возникла ошибка на шаге: Перемещаемся вверх 6.3.'
                    )
                    ;
                    $conn->rollbackTransaction();
            ...
            $sql = "UPDATE\n\t"
                   ...
                   . $arParams['skew_level'] . "\n"
                   . "WHERE\n\t"
                   . $helper->wrapFieldQuotes($sPrimaryFieldName) . " IN ("
                   . implode(',', $arParams['id_edit']) . ")";
            $query = new QueryBase($sql);
            try
            {
                ...
                if (!$res->isSuccess())
                {
                    $this->getErrorCollection()->add(
                        'GO_UP_6_4',
                        'Возникла ошибка на шаге: Перемещаемся вверх 6.4.'
                    )
                    ;
                    $conn->rollbackTransaction();
        ...
        else
        {
            ...
            try
            {
                    ...
                    //			if (false)
                {
                    $this->getErrorCollection()->add(
                        'GO_DOWN_6_2',
                        'Возникла ошибка на шаге: Перемещаемся вниз 6.2.'
                    )
                    ;
                    $conn->rollbackTransaction();
            ...
            try
            {
                    ...
                    //			if (false)
                {
                    $this->getErrorCollection()->add(
                        'GO_DOWN_6_3',
                        'Возникла ошибка на шаге: Перемещаемся вниз 6.3.'
                    )
                    ;
                    $conn->rollbackTransaction();
            ...
            try
            {
                    ...
                    //			if (false)
                {
                    $this->getErrorCollection()->add(
                        'GO_DOWN_6_4',
                        'Возникла ошибка на шаге: Перемещаемся вниз 6.4.'
                    )
                    ;
                    $conn->rollbackTransaction();
    ...
    /**
     * Пересоздает коллекцию ошибок, обнуляя ее
     *
     * @return $this
     * @unittest
     */
    ...
    /**
     * Возвращает значение свойства errorCollection
     *
     * @return null|ErrorCollection
     * @unittest
     */
    ...
    /**
     * Возвращает массив ошибок, либо NULL
     *
     * @return array|null
     * @unittest
     */
     ...
     * @unittest
     */
    final public function activateNode ($primary)
    {
     ...
     * @unittest
     */
    final public function addNode (&$arNode)
    {
        $conn = Application::getInstance()->getConnection();
        //OK
        /*		Создание узла – самое простое действие над деревом.
                Для того, что бы его осуществить нам потребуется уровень и
                правый ключ родительского узла (узел в который добавляется новый),
                либо максимальный правый ключ, если у
        ...
        */
        ...
        if (!$this->checkNodeFields($arNode))
        {
        ...
        /*		Пусть $right_key – правый ключ родительского узла, или максимальный
                ...
                максимальным правым ключом не будет обновляться, соответственно,
                чтобы небыло повторов, берем число на единицу большее).
                $level – уровень родительского узла, либо 0, если родительского
                нет.
        */
        /** @var IField $oPrimaryField */
        $oPrimaryField = $this->getPrimaryField();
        $sPrimaryFieldName = $oPrimaryField->getColumnName();
        // msDebug($sPrimaryFieldName);
        $sParentFieldName = $this->getTable()->getParentFieldName();
        // msDebug($sParentFieldName);
        if (
            (
                isset($arNode[$sParentFieldName])
                && strlen((string)$arNode[$sParentFieldName]) <= 0
            )
            || !isset($arNode[$sParentFieldName])
        )
        {
            ...
            if ($ar_res = $res->fetch())
            {
                $right_key = (int)$ar_res['RIGHT_MARGIN'] + 1;
                $level = 0;
            }
            else
            {
        ...
        {
            $arParent = $this->getByPrimary(
                $arNode[$sParentFieldName],
                ['RIGHT_MARGIN', 'LEFT_MARGIN', 'DEPTH_LEVEL']
            );
            if ($arParent)
            {
            ...
            try
            {
                $res = $query->exec();/*			Но мы обновили только те узлы в которых изменяются оба
                            ключа, при этом родительскую ветку (не узел, а все
                            родительские узлы) мы не трогали, так как в них изменяется
                            только правый ключ. Следует иметь в виду, что
                            если у нас не будет родительского узла, то есть новый
                            узел будет корневым, то данное обновление проводить
                            нельзя.*/
                if (!$res->isSuccess())
                {
                    $this->getErrorCollection()->add(
                        'UPDATE_TREE_KEYS',
                        'Возникла ошибка на шаге: 1. Обновляем ключи '
                        . 'существующего дерева, узлы стоящие за родительским узлом'
                    )
                    ;
                    $conn->rollbackTransaction();
        ...
        try
        {
            ...
            {
                $this->getErrorCollection()->add(
                    'UPDATE_PARENT_BRANCH', 'Возникла ошибка на шаге: 2. '
        ...
        if (
            ...
            && ($oPrimaryField instanceof IntegerField)
        ...
        try
        {
            ...
            else
            {
                $this->getErrorCollection()->add(
                    'ADD_NEW_NODE',
                    'Возникла ошибка на шаге: 3. Теперь добавляем новый узел'
                )
                ;
                $conn->rollbackTransaction();
    ...
    final public function checkTable ()
    {
         ...
         * 5. Если уровень узла нечетное число то тогда левый ключ ВСЕГДА
         * нечетное число, то же самое и для четных чисел;
         * 6. Ключи ВСЕГДА уникальны, вне зависимости от того правый он или левый;
         */
        $bError = false;
        $arResult = [];
        $sPrimaryFieldName = $this->getPrimaryFieldName();
        $helper = new SqlHelper($this->getTable()->getTableName());

        //1. Левый ключ ВСЕГДА меньше правого;
        //Если все правильно то результата работы запроса не будет, иначе,
        ...
        //2. Наименьший левый ключ ВСЕГДА равен 1;
        //3. Наибольший правый ключ ВСЕГДА равен двойному числу узлов;
        //Получаем количество записей (узлов), минимальный левый ключ и
        // максимальный правый ключ, проверяем значения.
        $sql2 = "SELECT\n\t"
        ...
        $sql4 = "SELECT\n\t"
                ...
                . "MOD((" . $helper->wrapFieldQuotes('RIGHT_MARGIN') . " - "
                . $helper->wrapFieldQuotes('LEFT_MARGIN')
                . "), 2) AS REMAINDER\n "
                . "FROM\n\t"
                . $helper->wrapTableQuotes() . "\n"
        ...
        //5. Если уровень узла нечетное число то тогда левый ключ ВСЕГДА нечетное
        // число, то же самое и для четных чисел;
        //Если все правильно то результата работы запроса не будет, иначе,
        // получаем список идентификаторов неправильных строк;
        $sql5 = "SELECT\n\t"
                . $helper->wrapQuotes($sPrimaryFieldName) . ",\n\t"
                . "MOD((" . $helper->wrapFieldQuotes('LEFT_MARGIN') . " - "
                . $helper->wrapFieldQuotes('DEPTH_LEVEL')
                . " + 2), 2) AS REMAINDER \n"
                . "FROM\n\t"
                . $helper->wrapTableQuotes() . "\n"
        ...
        /*
            ...
            по сути осуществляется из одной таблицы,
            но в разделе FROM эта таблица "виртуально" продублирована 3 раза: из
            первой мы выбираем все записи по порядку и начинаем сравнивать с
            записями второй таблицы (раздел WHERE) в результате мы получаем все записи
            неповторяющихся значений. Для того, что бы определить сколько раз
            запись не повторялась в таблице, производим группировку (раздел
            GROUP BY) и получаем число "не повторов" (COUNT(t1.id)). По условию,
            если все ключи уникальны, то число не повторов будет меньше на одну
            единицу чем общее количество записей. Для того, чтобы определить
            количество записей в таблице, берем максимальный правый ключ
            (MAX(t3.right_key)), так как его значение - двойное число записей,
            но так как в условии отбора для записи с максимальным правым ключом -
            максимальный правый ключ будет другим, вводится третья таблица, при
            этом число "неповторов" увеличивается умножением его на количество
            записей. SQRT(4*rep +1) - решение уравнения x^2 + x = rep.
            Если все правильно то результата работы запроса не будет, иначе,
            получаем список идентификаторов неправильных строк;
         */
        $sql6 = "SELECT\n\t"
                ...
                . "WHERE\n\t"
                . "t1." . $helper->wrapQuotes('LEFT_MARGIN')
                . " <> t2." . $helper->wrapQuotes('LEFT_MARGIN')
                . " AND\n\t"
                . "t1." . $helper->wrapQuotes('LEFT_MARGIN')
                . " <> t2." . $helper->wrapQuotes('RIGHT_MARGIN')
                . " AND\n\t"
                . "t1." . $helper->wrapQuotes('RIGHT_MARGIN')
                . " <> t2." . $helper->wrapQuotes('LEFT_MARGIN')
                . " AND\n\t"
                . "t1." . $helper->wrapQuotes('RIGHT_MARGIN')
                . " <> t2." . $helper->wrapQuotes('RIGHT_MARGIN') . "\n"
     ...
     * @param array $arNode Массив полей узла
     *
     * @see TreeORMController::changeParent ($nodeID, $newParentID=null)
     *
     * @see TreeORMController::sortNode ($nodeID, $prevNode=0)
     *
     * Для изменения уровня вложенности
     * @unittest
     */
    final public function checkUpdateFields (&$arNode)
    {
        //Левая граница
     ...
     * @unittest
     */
    final public function deactivateNode ($primary)
    {
     ...
     * @unittest
     */
    final public function deleteNode ($primary)
    {
        $conn = Application::getInstance()->getConnection();
        $this->clearErrorCollection();

        /*		Удаление узла не намного сложнее, но требуется учесть, что у
                удаляемого узла могут быть подчиненные узлы. Для
                осуществления этого действия нам потребуется левый и правый ключ
                удаляемого узла.
        */
        ...
        try
        {
            ...
            if (!$res->isSuccess())
            {
                $this->getErrorCollection()->add(
                    'DELETE_BRANCH',
                    'Возникла ошибка на шаге: 1. Удаляем узел (ветку)'
                )
                ;
        ...
        /*		Как и в случае с добавлением обновление происходит двумя
                командами: обновление ключей родительской ветки и
                обновление ключей узлов, стоящих за родительской веткой. Следует
                правда учесть, что обновление будет
        ...
        */
        ...
        try
        {
            ...
            if (!$res->isSuccess())
            {
                $this->getErrorCollection()->add(
                    'DELETE_BRANCH',
                    'Возникла ошибка на шаге: 2. Обновляем ключи оставшихся веток'
                )
                ;
                $conn->rollbackTransaction();
        ...
        try
        {
            ...
            if (!$res->isSuccess())
            {
                $this->getErrorCollection()->add(
                    'DELETE_BRANCH',
                    'Возникла ошибка на шаге: 2.2. Обновление последующих узлов'
                )
                ;
                $conn->rollbackTransaction();
     ...
     * @unittest
     */
    final public function getBranch ($primary, $arSelect = [], $bActive = false)
    {
        $arNode = $this->getByPrimary(
            $primary,
        ...
        if ($bActive)
        {
            $arGetList['filter'] = array_merge(
                $arGetList['filter'],
                ['ACTIVE' => true]
            );
        }
        if ($arResult = $this->getList($arGetList))
        {
            return $arResult;
        }
        else
     ...
     * @param int        $iDepthLevel Если > 0 - будут выбраны только указанного
     *                                уровня вложенности
     *
     * @return bool|array
     * @throws SqlQueryException
     ...
     *                              будут выбраны поля по-молчанию:
     *                              [PRIMARY], 'LEFT_MARGIN', 'RIGHT_MARGIN',
     *                              'DEPTH_LEVEL', [PARENT_PRIMARY] Для получения
     *                              имен полей PRIMARY и PARENT_PRIMARY
     *                              будут выполнены соответствующие методы
     *
     * @return array|bool
     * @throws SqlQueryException
     * @throws \Ms\Core\Exceptions\Arguments\ArgumentTypeException
     *
     * @see ORMController::getPrimaryFieldName()
     * @see TreeTableAbstract::getParentFieldName()
     * @unittest
     */
    final public function getNodesByDepthLevel ($iDepthLevel = 1, $arSelect = [])
    {
        $iDepthLevel = (int)$iDepthLevel;
        if ($iDepthLevel <= 0)
        {
        ...
        if (empty($arSelect))
        {
            $arSelect = $this->getArSelect();
        }
        else
        {
            $arSelect = array_merge($arSelect, $this->getArSelect());
     ...
     * @unittest
     */
    final public function getParentInfo ($primary, $arSelect = [])
    {
        //OK
        $arNode = $this->getByPrimary(
            $primary,
        ...
        if ($arResult = $this->getOne($arGetOne))
        {
            return $arResult;
        }
        else
     ...
     * @unittest
     */
    final public function getParentLevel ($primary)
    {
        //OK
        if ($arResult = $this->getParentInfo($primary, ['DEPTH_LEVEL']))
        {
     ...
     * @unittest
     */
    final public function getParentPrimary ($primary)
    {
        //OK
        $sPrimaryName = $this->getPrimaryFieldName();
        if ($arResult = $this->getParentInfo($primary, [$sPrimaryName]))
        {
     ...
     * @param bool|array $arSelect        Массив возвращаемых полей
     * @param bool       $bActive         Вернуть только активные узлы
     * @param bool       $bReturnYourself Вернуть в том числе данные о самом
     *                                    узле
     * @param bool       $bReverse        Флаг: вывести в обратном направлении
     *                                    (от себя и ближайшего родителя к корню)
     *
     * @return bool|array
     * @throws SqlQueryException
     * @throws \Ms\Core\Exceptions\Arguments\ArgumentTypeException
    ...
    final public function getParents (
        ...
        $arGetList = [
            'filter' => [
                '<' . ($bReturnYourself ? '=' : '') . 'LEFT_MARGIN'  =>
                    $arNode['LEFT_MARGIN'],
                '>' . ($bReturnYourself ? '=' : '') . 'RIGHT_MARGIN' =>
                    $arNode['RIGHT_MARGIN']
            ],
     ...
     * @unittest
     */
    final public function getTreeList ($arSelect = [], $bActive = false)
    {
        $arGetList = [
            'order' => ['LEFT_MARGIN' => 'ASC']
        ];
     ...
     * @unittest
     */
    final public function sortNode ($primary, $sort = 'LEFT_MARGIN', $order = 'ASC')
    {
        $conn = Application::getInstance()->getConnection();
        ...
        */
        ...
        $arNode = $this->getByPrimary($primary, null, $arSelect);
        //		msDebug($arNode);
        $helper = new SqlHelper(static::getTableName());
        //</editor-fold>
        ...
        $arParentNode = $this->getParentInfo($primary, $arSelect);
        ...
        if ($arParentNode)
        {
            $arChild = $this->getChildren($arParentNode[$sPrimaryFieldName], $arSelect);
            $arParent = $arChild[0];
            unset($arChild[0]);
        ...
        {
            $arChild = $this->getNodesByDepthLevel(1, $arSelect);
        }
        ...
        //Получаем $arParams['right_key_near'] и $arParams['left_key_near']
        // (для варианта изменения порядка)
        //Если элемент должен находится в самом низу и находится там - выходим,
        // так как ничего делать не нужно
        ...
        if (!empty($arParams['id_edit']) && $oPrimaryField instanceof StringField)
        {
            $arTmp = $arParams['id_edit'];
            $arParams['id_edit'] = [];
        ...
        if (($arParams['left_key_near'] < $arParams['left_key']) && ($arParams['level'] > $arParams['level_near']))
        {
            ...
            $sql = "UPDATE\n\t"
                   ...
                   . $arParams['left_key'] . " AND\n\t"
                   . $helper->wrapQuotes('RIGHT_MARGIN') . " > "
                   . $arParams['left_key_near'] . " AND\n\t"
                   . $helper->wrapQuotes($sPrimaryFieldName) . " NOT IN ("
                   . implode(',', $arParams['id_edit']) . ")";
            $query = new QueryBase($sql);
            try
            {
                ...
                if (!$res->isSuccess())
                {
                    $this->getErrorCollection()->add(
                        'SORT_UP_PARENT_1',
                        'Возникла ошибка на шаге: Если перемещаемся выше и '
                        . 'встаем сразу за родительским узлом 1'
                    )
                    ;
                    $conn->rollbackTransaction();
            ...
            */
            ...
            $sql = "UPDATE\n\t"
                   ...
                   . $arParams['skew_edit'] . "\n"
                   . "WHERE\n\t"
                   . $helper->wrapQuotes($sPrimaryFieldName) . " IN ("
                   . implode(',', $arParams['id_edit']) . ")";
            $query = new QueryBase($sql);
            try
            {
                ...
                if (!$res->isSuccess())
                {
                    $this->getErrorCollection()->add(
                        'SORT_UP_PARENT_2',
                        'Возникла ошибка на шаге: Теперь можно переместить ветку'
                    )
                    ;
        ...
        elseif ($arParams['left_key_near'] < $arParams['left_key'])
        {
            ...
            // $right_key_near - $left_key + 2 = $skew_edit;
            $arParams['skew_edit'] =
                ($arParams['left_key_near'] - $arParams['left_key'] + 2)
                * (-1);
            ...
            try
            {
                ...
                if (!$res->isSuccess())
                {
                    $this->getErrorCollection()->add(
                        'SORT_UP_1',
                        'Возникла ошибка на шаге: Если перемещаемся выше'
                    )
                    ;
        ...
        else
        {
            ...
            //Определяем смещение ключей редактируемого узла
            // $right_key_near - $left_key + 1 - $skew_tree = $skew_edit.
            $arParams['skew_edit'] =
                $arParams['right_key_near']
                - $arParams['left_key']
                + 1
            ...
            */
            ...
            try
            {
                ...
                if (!$res->isSuccess())
                {
                    $this->getErrorCollection()->add(
                        'SORT_DOWN_1',
                        'Возникла ошибка на шаге: Если перемещаемся ниже'
                    )
                    ;
    ...
    /**
     * Проверяет наличие обязательных полей, удаляет неизменяемые поля.
     * Возвращает измененный массив полей раздела
     *
     * @param array $arNode Массив полей узла
     *
     * @return bool|array
     */
    final protected function checkNodeFields (array &$arNode)
    {
        if (is_null($this->getErrorCollection()))
        {
            $this->clearErrorCollection();
        }
        //OK
        /** @var IField $sPrimaryField */
        $sPrimaryField = $this->getPrimaryField();
        $sPrimaryFieldName = $sPrimaryField->getColumnName();
        if (is_null($arNode))
        {
            $this->getErrorCollection()->add(
                'EMPTY_NODE_FIELDS',
                'Массив полей узла не задан'
            )
            ;
        ...
        if (
            isset($arNode[$sPrimaryFieldName])
            && ($sPrimaryField instanceof IntegerField)
     ...
     *
     * @param array  $arNode   Массив полей узла
     * @param mixed &$arParams Массив параметров, куда будут записаны данные.
     *                         Передается по ссылке
     *
     * @api
     *
     */
    final protected function getKeysAndLevel (array $arNode, &$arParams)
    {
        $arParams['level'] = $arNode['DEPTH_LEVEL'];
        $arParams['left_key'] = $arNode['LEFT_MARGIN'];
        $arParams['right_key'] = $arNode['RIGHT_MARGIN'];
    ...
    /**
     * Возвращает массив полей узла по-умолчанию. При сборе массива вызываются
     * методы, возвращающие Ключ таблицы и Ключ родительского узла
     *
     * @uses ORMController::getPrimaryFieldName()
     * @uses TreeORMController::getParentFieldName()
     */
    private function getArSelect ()
    {
        $arReturn = [$this->getPrimaryFieldName()];
        $arReturn = array_merge($arReturn, self::$defaultArSelect);
        $arReturn[] = $this->getTable()->getParentFieldName();
