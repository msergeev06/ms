...
class Errors implements IAllErrors
{
    ...
    const ERROR_MODULE_NOT_EXIST = 106;

    const ERROR_CLASS_WRONG_NAME = 110;
    const ERROR_CLASS_NOT_AUTOLOAD = 111;
    const ERROR_OBJECT_NOT_INSTANCE_OF_CLASS = 112;
    const ERROR_CLASS_NOT_FOUND = 113;
    const ERROR_METHOD_NOT_FOUND = 114;
    const ERROR_METHOD_NOT_PUBLIC = 115;

    const ARGUMENT_EXCEPTION = 200;
    const VALIDATION_EXCEPTION = 210;
    const COMPONENT_EXCEPTION = 220;
    const ACCESS_EXCEPTION = 230;

    /**
     * Возвращает ошибку в виде строки [Код_ошибки] Текст ошибки
     *
     * @param int   $iErrorCode Код ошибки
    ...
    public static function getErrorTextByCode($iErrorCode, $arReplace = []): string
    {
        switch ((int)$iErrorCode)
        {
            ...
            case self::ERROR_CLASS_NOT_AUTOLOAD:
                ...
                {
                    $arReplace['MODULE_NAME'] = Modules::getInstance()->getModuleFromNamespace(
