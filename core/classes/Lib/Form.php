...
use Ms\Core\Entity\System\Localization\Loc;
...
class Form
{
    ...
    public static function checkInputText($value)
    {
        if (preg_match("/[\<|\>]/", $value))
        {
            return Loc::getInstance()->getCoreMessage('error_wrong_symbols');
        }
        else
        {
            return true;
    ...
    public static function checkInputNumber($value, $step = false, $min = false, $max = false)
    {
        ...
        if ($step !== false)
        {
            ...
            {
                return Loc::getInstance()->getCoreMessage('error_step', ['STEP' => $step]);
            }
        }
        if ($min !== false)
        {
            ...
            {
                return Loc::getInstance()->getCoreMessage('error_bigger_or_equal', ['MIN' => $min]);
            }
        }
        if ($max !== false)
        {
            ...
            {
                return Loc::getInstance()->getCoreMessage('error_low_or_equal', ['MAX' => $max]);
            }
        }
    ...
    public static function checkInputDate(Date $value, Date $min = null, Date $max = null)
    {
        ...
        {
            return Loc::getInstance()->getCoreMessage(
                'error_date_between',
