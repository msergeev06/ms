...
class Files
{
    ...
    {
        $badDirs = [];
        $path = str_replace(["\\", "//"], "/", $sPath);
        $dirPerm = Application::getInstance()->getSettings()->getChmodDir();
    ...
    public static function copyDirFiles (
        $sPathFrom, $sPathTo, $bRewrite = true, $bRecursive = false, $bDeleteAfterCopy = false, $sExclude = ""
    ) {
        $filePerm = Application::getInstance()->getSettings()->getChmodFile();
        ...
        if ($handle = @opendir($sPathFrom))
        {
            while (($file = readdir($handle)) !== false)
            {
                ...
                {
                    static::copyDirFiles(
                        $sPathFrom . "/" . $file, $sPathTo . "/" . $file, $bRewrite, $bRecursive, $bDeleteAfterCopy,
                        $sExclude
                    );
                    if ($bDeleteAfterCopy)
                    {
     ...
     * @return array|bool
     */
    public static function getListFiles ($dir, $arIgnore = [])
    {
        if (!file_exists($dir))
        {
            return false;
