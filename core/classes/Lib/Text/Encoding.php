...
/**
 * Класс Ms\Core\Lib\Text\Encoding
 * Работа с кодировками
 */
class Encoding
{
    const PATH_TO_CONVERT_TABLES = "/ms/core/cvtables/";
    ...
    public static function convertEncodingToCurrent ($string)
    {
        ...
        {
            $currentCharset = Application::getInstance()->getSettings()->getCharset();
        }
    ...
    protected function buildConvertTable ()
    {
        ...
        for ($i = 0, $cnt = func_num_args(); $i < $cnt; $i++)
        {
            ...
            $pathToTable = Application::getInstance()->getDocumentRoot() . self::PATH_TO_CONVERT_TABLES . $fileName;
            ...
            {
                $this->errors[] =
                    new Error(str_replace("#FILE#", $pathToTable, "Can not open file #FILE# for reading."));
    ...
    {
        $res = '';
        if (Application::getInstance()->getSettings()->isDisableIconv() !== true)
        {
            $utfString = false;
            ...
            if (function_exists('iconv'))
            {
                ...
                {
                    $this->errors[] =
                        new Error("Iconv reported failure while converting string to requested character encoding.");
                }
            }
            elseif (function_exists('libiconv'))
            {
                ...
                {
                    $this->errors[] =
                        new Error("Libiconv reported failure while converting string to requested character encoding.");
                }
            }
        }
    ...
    protected function convertByMbstring ($data, $charsetFrom, $charsetTo)
    {
        ...
        if (
            extension_loaded("mbstring") && @mb_encoding_aliases($charsetFrom) !== false
            && @mb_encoding_aliases(
                $charsetTo
            ) !== false
        )
                ...
                if ($charsetTo != "utf-8")
                {
                        ...
                        for ($j = 0; $j < $len; $j++)
                        {
                            ...
                            else
                            {
                                $this->errors[] = new Error(
                                    str_replace(
                                        "#CHAR#", $sourceString[$i],
                                        "Cannot find matching char \"#CHAR#\" in destination encoding table."
                                    )
                                );
                            }
                        }
                    ...
                    else
                    {
                        $this->errors[] = new Error(
                            str_replace(
                                "#CHAR#", $sourceString[$i],
                                "Cannot find matching char \"#CHAR#\" in source encoding table."
                            )
                        );
                    }
                }
    ...
    protected function hexToUtf ($utfCharInHex)
    {
        ...
        {
            $result .= chr(($utfCharInDec >> 12) + 224) . chr((($utfCharInDec >> 6) & 63) + 128) . chr(
                    ($utfCharInDec & 63) + 128
                );
        }
        elseif ($utfCharInDec < 2097152)
        {
            $result .= chr($utfCharInDec >> 18 + 240) . chr((($utfCharInDec >> 12) & 63) + 128) . chr(
                    ($utfCharInDec >> 6) & 63 + 128
                ) . chr($utfCharInDec & 63 + 128);
        }
