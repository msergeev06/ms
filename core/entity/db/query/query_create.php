<?php
/**
 * MSergeev\Core\Entity\Db\Query\QueryCreate
...
class QueryCreate extends QueryBase
{
	...
	private function getDefaultValue (Fields\ScalarField $objData)
	{
		...
		//Получаем значение по-умолчению для булевого типа поля
		/* * @var Fields\BooleanField $objData * /
		if ($objData instanceof Fields\BooleanField)
		{
			//Если существует значение по умолчанию и значение обязательное
			if (!is_null($objData->getDefaultValueDB()) && $isNotNull)
			{
				$sql .= "NOT NULL DEFAULT '".$objData->getDefaultValueDB()."' ";
			}
			//Если значение обязательное
			elseif ($isNotNull)
			{
		...
		//Получаем значение по-умолчанию для поля даты
		/** @var Fields\DateField $objData * /
		elseif ($objData instanceof Fields\DateField)
		{
			if (!is_null($objData->getDefaultValue('create')) && $isNotNull)
			{
				$sql .= 'NOT NULL ';
				...
				{
					/** @var Date $date * /
					$date = $objData->getDefaultValue('create');
					$sql .= "DEFAULT '".$date->getDateDB()."' ";
				}
			}
			//Если значение обязательное
			elseif ($isNotNull)
		...
		//Получаем значение по-умолчанию для остальных типов полей
		/** @var Fields\ScalarField $objData * /
		else
		{
			if (!is_null($objData->getDefaultValue('create')) && $isNotNull)
