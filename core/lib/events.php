...
class Events
{
	 ...
	 * @var array
	 */
	private static $arEventsModules = array();
	 ...
	 * Если указан полный путь к файлу с обработчиком fullPath, то он будет автоматически подключен перед вызовом
	...
	public static function addEventHandler($fromModule, $eventID, $callback, $sort=100, $fullPath=false)
	{
		...
		self::$arEvents[$fromModule][$eventID][$sort][$hash] = array(
			'CALLBACK' => $callback,
			'FULL_PATH' => $fullPath
		);
	}
	 ...
	 * @param string    $toClass        Класс принадлежащий модулю $toModule, метод которого является
	 *                                      функцией-обработчиком события.
	 *                                      Необязательный параметр. По умолчанию - ""
	 *                                      (будет просто подключен файл /ms/modules/$toModule/include.php)
	...
	public static function registerModuleDependences (
		$fromModule, $eventID, $toModule='', $toClass = "", $toMethod = "", $sort = 100, $toPath="", $fullPath="",
		$toMethodArg=array()
	)
	{
		$arAdd = array(
		...
		if ($toPath != '' && file_exists(Application::getInstance()->getSettings()->getMsRoot().$toPath))
		{
	...
	public static function executeModuleEvent ($arEvent, $arParams=array())
	{
		...
		elseif(
			...
			&& $arEvent["TO_PATH"]<>""
			&& file_exists(Application::getInstance()->getSettings()->getMsRoot().$arEvent["TO_PATH"])
		)
		{
			$r = include_once(Application::getInstance()->getSettings()->getMsRoot().$arEvent["TO_PATH"]);
		}
