...
class File
{
	...
	private static function init ()
	{
		...
		{
			self::$documentRoot = Options::getOptionStr('DOCUMENT_ROOT',$app->getDocumentRoot());
		}
		if (is_null(self::$uploadDir))
		{
			self::$uploadDir = Options::getOptionStr('UPLOAD_DIR',$app->getSettings()->getUploadDir());
		}
	}
	...
	public static function addNewImg ($moduleName, array $arFile)
	{
		...
		Events::runEvents('core','OnBeforeUploadNewFile',array(&$arFile, &$arAdd));
		...
		Events::runEvents('core','OnAfterUploadNewFile',array(&$arFile, &$arAdd));

		Events::runEvents('core','OnBeforeAddNewFile',array(&$arFile,&$arAdd));

		$res = Tables\FileTable::add($arAdd);
		if ($res->getResult())
		{
			Events::runEvents('core','OnAfterAddNewFile',array($arFile,$res->getInsertId()));
	...
	public static function showImage ($strImage, $maxWidth=0,$maxHeight=0,$sParams=null,$imageUrl='',$bPopup=false,$popupTitle=false,$sizeWHTTP=0,$sizeHHTTP=0)
	{
		...
		else
		{
			...
			else
			{
				...
				Application::getInstance()->addJsToDownPage("$('.popup-link-".$iImageID."').magnificPopup({type: 'image'});");
			}
		}
		return $strReturn;
	}
		...
		if (!is_null($fileID) && intval($fileID)>0)
		{
			if ($arFile = self::getFileArray($fileID))
			{
				if (file_exists($arFile['FILE_PATH']))
				{
					...
					if (!file_exists($arFile['FILE_PATH']))
					{
						$res = Tables\FileTable::delete($arFile['ID'],true);
						if ($res->getResult())
						{
							return true;
						}
						...
						{
							$res = Tables\FileTable::update($arFile['ID'],array('EXTERNAL_ID'=>'DELETE'));
							if ($res->getResult())
							{
								return true;
							}
